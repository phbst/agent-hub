...
function Agents({ agents, onChange }: { agents: AgentRecord[]; onChange: () => Promise<void> }): React.ReactElement {
  ...
  const [error, setError] = useState("");
  const visibleAgents = agents.filter((agent) => showRevoked || agent.status !== "revoked");
  ...
  const remove = (agent: AgentRecord) => {
    if (window.confirm(`彻底删除 ${agent.name}？其凭证一并销毁，任务历史保留但不再关联该 Agent。`)) void action({ action: "remove", agent_id: agent.id });
  };
  ...
  return <><div className="toolbar"><button onClick={() => void generate()}><CirclePlus size={17} />生成注册令牌</button><label className="inline-field">有效<input type="number" min={5} max={1440} value={minutes} onChange={(event) => setMinutes(Number(event.target.value))} />分钟</label><label className="inline-field">可用<input type="number" min={1} max={100} value={uses} onChange={(event) => setUses(Number(event.target.value))} />次</label>{bootstrap && <div className="token-output"><code>{bootstrap}</code><button className="icon-button" title="复制" onClick={() => void navigator.clipboard.writeText(bootstrap)}><Clipboard size={16} /></button></div>}{revokedCount > 0 && <label className="toggle"><input type="checkbox" checked={showRevoked} onChange={(event) => setShowRevoked(event.target.checked)} />显示已吊销 ({revokedCount})</label>}</div>{error && <div className="notice error">{error}</div>}<section className="table-panel"><div className="agent-row header"><span>Agent</span><span>模式与标签</span><span>负载</span><span>心跳</span><span>操作</span></div>{visibleAgents.map((agent) => { const live = agent.status === "online" && Date.now() - new Date(agent.last_heartbeat ?? 0).getTime() < 90_000; return <div className="agent-row" key={agent.id}><div><strong>{agent.name}</strong><span className={`status ${live ? "online" : agent.status}`}>{live ? "online" : agent.status}</span>{agent.paused && <span className="status pending">已暂停</span>}</div><div><span className="mode">{agent.mode}</span> {agent.labels.join(", ") || "无标签"}</div><div>{agent.running_count}/{agent.max_concurrency}</div><div>{age(agent.last_heartbeat)}</div><div className="actions">{agent.status === "pending_approval" && <button onClick={() => void action({ action: "approve", agent_id: agent.id })}>批准</button>}{agent.status !== "revoked" && agent.status !== "pending_approval" && <button onClick={() => void togglePause(agent)}>{agent.paused ? "恢复" : "暂停"}</button>}{agent.status !== "revoked" && <button onClick={() => setEditing(agent)}>编辑</button>}{agent.status !== "revoked" && <button className="danger" onClick={() => revoke(agent)}>吊销</button>}{agent.status === "revoked" && <button className="danger" onClick={() => remove(agent)}>删除</button>}</div></div>; })}{visibleAgents.length === 0 && <div className="empty">暂无 Agent</div>}</section>{editing && <EditAgent agent={editing} onClose={() => setEditing(null)} onSaved={async () => { setEditing(null); await onChange(); }} />}</>;
}
